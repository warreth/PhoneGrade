...
function initDebugOverlay() {
    ...
    overlay.style.cssText = 'position:fixed;bottom:0;left:0;right:0;height:30%;background:rgba(0,0,0,0.85);color:#0f0;font-family:monospace;font-size:10px;padding:10px;overflow-y:auto;z-index:9999;border-top:2px solid #0f0;';
    
    const title = document.createElement('div');
    title.innerText = 'PWA Network & Console Debugger (Tap to close)';
    title.style.cssText = 'font-weight:bold;color:#fff;margin-bottom:8px;text-align:center;cursor:pointer;';
    title.onclick = () => overlay.style.display = 'none';
    ...
    function logToOverlay(msg, isError = false) {
    ...
    console.log = function(...args) {
        logToOverlay(args.map(a => typeof a === 'object' ? JSON.stringify(a) : a).join(' '));
        origConsoleLog.apply(console, args);
    };
    console.error = function(...args) {
        logToOverlay(args.map(a => typeof a === 'object' ? JSON.stringify(a) : a).join(' '), true);
    ...
    window.fetch = async function(...args) {
        ...
        let payload = (args[1] && args[1].body) || '';
        if (typeof payload === 'string' && payload.length > 100) payload = payload.substring(0,100) + '...';
        
        logToOverlay(`req: ${method} ${url} ${payload}`);
        try {
            const resp = await origFetch.apply(this, args);
            logToOverlay(`res: ${method} ${url} -> ${resp.status}`, !resp.ok);
            return resp;
        } catch (err) {
            logToOverlay(`err: ${method} ${url} -> ${err.message}`, true);
            throw err;
        }
    };
    
    logToOverlay('Debug overlay initialized.');
}
initDebugOverlay();
